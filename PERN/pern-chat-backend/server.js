...
app.use(express.urlencoded(
    {extended : true})); // to be able to receive data from frontend
...
io.on("connection", (socket) => {
  // Handle a 'new-user' event by emitting the current members to all connected
  // clients
  socket.on("new-user", async () => {
    const members = await knex(USER_TABLE_NAME).select(`${USER_TABLE_NAME}.*`);
    io.emit("new-user", members);
  });
  // Handle a 'join-room' event by having the socket join the specified room and
  // levaing the previous room
  ...
  ``
  // Handle a 'message-room' event by inserting the message into the database
  // and emitting it to the specified room
  socket.on("message-room", async (room, content, sender, time, date) => {
    ...
    // Broadcasting the 'new-messages' event to all the rooms except current
    // room.
