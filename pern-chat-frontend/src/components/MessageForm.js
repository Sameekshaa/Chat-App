    ...
    return (
        <>
            <div className="messages-output">
                {user && !privateMemberMsg?.id && <div className="alert alert-info">You are in the {currentRoom} room</div>}
                                ...
                                <div className={sender?.email === user?.email ? "message" : "incoming-message"} key={msgIdx}>
                                    <div className="message-inner">
                                        <div className="d-flex align-items-center mb-3">
                                            <img src={sender.picture} style={{ width: 35, height: 35, objectFit: "cover", borderRadius: "50%", marginRight: 10 }} alt="sender-pic" />
                                            <p className="message-sender">{sender.id === user?.id ? "You" : sender.name}</p>
                                        </div>
